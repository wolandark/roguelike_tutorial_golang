...
>>> 2. In `gamemap.go`, add the fields `Visible [][]bool` and `Explored [][]bool` to `GameMap`, and allocate their rows in `NewGameMap` in the same loop as the rows of `Tiles`.
...
>>> 5. Add a method `func (m *GameMap) ComputeFOV(ox, oy, radius int)` that clears `Visible` (call `clear(row)` on each row) and marks every cell within the square of `radius` around `ox, oy`.
...
- `for _, row := range m.Visible { clear(row) }` resets every row to `false`. `clear` is a built-in (Go 1.21+) that sets every element of a slice to its zero value. `row` is a copy of the row's slice header, but it points at the same cells, so clearing it clears the grid.
...
%%% Remove the `clear(row)` loop from `ComputeFOV`. Everything you have ever seen stays lit: visibility must be recomputed from scratch each turn, memory is what persists.
