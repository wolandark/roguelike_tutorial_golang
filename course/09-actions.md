...
- `type Action interface{}`: an **interface** lists methods a value must have; an empty one lists none, so *any* value is an `Action`. For now it is only a common return type; in step 16 it gets a real method.
