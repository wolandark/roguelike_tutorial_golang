...
%%% Ask for `gameMap.TileAt(-1, 0)` somewhere and run: no panic, you silently get the last tile of the previous row, because `-1 + 0*80` is `-1`... which *does* panic. Now try `TileAt(-1, 1)`: index 79, the end of row 0, no panic, wrong tile. That is why `InBounds` exists and why step 14 checks it first.
