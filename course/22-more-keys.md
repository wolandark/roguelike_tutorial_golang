...
Four arrows cannot move diagonally, and roguelike players expect `hjkl` with `yubn` for diagonals, the numpad, and Home/End/PgUp/PgDn. That is twenty bindings; a `switch` with twenty cases is tedious and adding one means finding the right spot. A map from key to direction is shorter, and adding a binding is one line. Two maps, because tcell reports special keys through `Key()` and printable characters through `Rune()`. Waiting a turn also needs a key, and therefore an action.
