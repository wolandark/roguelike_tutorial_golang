...
func (TakeStairsAction) Perform(engine *Engine, entity *Entity) error {
	...
	if recovered := entity.Fighter.Heal(entity.Fighter.MaxHP / 4); recovered > 0 {
		engine.Log(fmt.Sprintf("You rest for a moment and recover %d HP.", recovered), colorHealthRecovered)
	}
	return nil
}
...
func (PickupAction) Perform(engine *Engine, entity *Entity) error {
	...
	for _, item := range engine.GameMap.Entities {
		if item.Consumable == nil || item.X != entity.X || item.Y != entity.Y {
			continue
		}
