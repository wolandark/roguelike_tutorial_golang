...
func placeEntities(room RectangularRoom, dungeon *GameMap, maxMonsters int) {
	n := rand.IntN(maxMonsters + 1)
	for i := 0; i < n; i++ {
		x := room.X1 + 1 + rand.IntN(room.X2-room.X1-1)
		y := room.Y1 + 1 + rand.IntN(room.Y2-room.Y1-1)
		if dungeon.GetBlockingEntityAt(x, y) != nil {
			continue
		}
		if rand.Float64() < 0.8 {
...
func GenerateDungeon(dungeon *GameMap, maxRooms, roomMinSize, roomMaxSize, maxMonstersPerRoom int, player *Entity) {
	...
	for i := 0; i < maxRooms; i++ {
		...
		placeEntities(newRoom, dungeon, maxMonstersPerRoom) // your turn! not in the first room
