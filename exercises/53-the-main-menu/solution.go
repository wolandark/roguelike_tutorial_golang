...
func (e *Engine) SaveAs(path string) error {
	data := saveData{Version: saveVersion, GameMap: e.GameMap, MessageLog: e.MessageLog, PlayerIndex: -1}
...
func LoadGame(path string) (*Engine, error) {
	...
	if data.Version != saveVersion {
		return nil, fmt.Errorf("save file version %d, want %d", data.Version, saveVersion)
	}
